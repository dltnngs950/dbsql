...
****알아 둬야 한다(못 외워도 찾아서 사용할 수 있으면 된다)
    모델링 툴을 사용하여 설계시 툴에서 설계된 테이블을 생성하는 구문을 자동으로 만들어준다.
...
2. 문자 : VARCHAR2(사이즈-byte), CHAR(X)
         ...
         VARCHAR(2000) : 최대 2000byte를 담을 수 있는 문자 타입
         java - 2byte, oracle ex 11g - 3byte
         
         CHAR(1~2000) : 고정 길이 문자열
         CHAR(5) 'test'
         test는 4byte고 char(5)는 5byte이기 때문에
...
4. Large OBJECT
    4.1 CLOB : 문자열을 저장할 수 있는 타입 = 사이즈 : 최대 4GB
    2.2.bOB : 바이너리 데이터, 시이즈 :4GB
       - CMS - 월 2만원
...
PRIMARY KEY : UNIQUE (중복된 값이 발생할 수 없다)
               + NOT NULL(널 값이 허용되지 않는다)

dept_test 테이블에 데이터가 없는상태

PRIMARY KEY 제약에 의해 deptno 컬럼에는 NULL값이 들어갈 수 없다.
INSERT INTO dept VALUES (NULL, 'ddit', 'deajeon');

50번 부서는 존재하지 않고 NULL값이 아니므로 정상적으로 등록
INSERT INtO dept VALUES (90, 'ddit', 'daejoen');

unique constraint (Soohun.SYS_c007084) violated(유니크 제약조건이 위배됨)
90번 부서가 이미 존재 하는 상태였기 때문에 PRIMARY KEY제약에 의해 정상적으로 입력될 수 없다.
...
INSERT INTO dept_test VALUES (90, 'ddit', 'deajeon');
INSERT INTO dept_test VALUES (90, 'ddit', 'deajeon');


2. 테이블 생성시 레이블 레벨로 제약조건 생성
CREATE TABLE 테이블명(
    컬럼1 컬럼1의 데이터타입,
    컬럼2 컬럼2의 데이터 타입,
    [TABLE LEVEL 제약조건]
...
PRIMARY KEY (deptno, dname) 설저에 따라 데티어가 입력될 수 있다.

복합 컬럼에 대한 제약조건은 컬럼 레벨에서는 설정이 불가하고
테이블 레벨, 혹은 테이블 생서후 제약조건을 추가하는 형태에서만 가능
INSERT INTO dept_test VALUES (90, 'ddit', 'daejeon');
INSERT INTO dept_test VALUES (90, 'ddit2', 'daejeon');

INSERT INTO dept_test VALUES (90, 'ddit2', 'daejeon');
( unique constraint (SOOHUN.PK_DEPT_TEST) violated 오류);
...
CREATE TABLE dept_test(
deptno NUMBER CONSTRAINT PK_dept_test PRIMARY KEY,
...
참조 무경성 제약조건에 의해 emp_test 테이블의 deptno 컬럼의 값은
dept_test 테이블의 deptno 컬럼에 존재하는 값만 입력이 가능하다

현재는 dept_tset 테이블에는 90번의 부서만 존재,
그렇게 때문에 emp_test 에는 90번 이외의 값이 들어갈 수가 없다.
INSERT INTO emp_test VALUES ( 9000, 'brown', 90);
INSERT INTO emp_test VALUES ( 9001, 'sally', 10); --10번이기때문에 안들어감.

테이블 레벨 참조 무결성 제약조건 생성
DROP TABLE emp_test;
FK_소스테이블_참조테이블 (소스테이블 : 내가 지금 작업하는 테이블)
...
입력시 : 부모(dept) ==> 자식(emp)
삭제시 : 자식(emp) ==> 부모(dept)   --삭제 할 때에는 부모보단 자식을 먼저 보자.
...
ROLLBACK;
SELECT *
FROM emp_test; --롤백이 되는 것은 DML 이다.
...
1. NOT NULL을 제외한 제약조건을 복사하진 않는다.
2. 개발시
...
2. 기존에 존재하는 컬럼의 변경 (이름, 데이터 타입)
   **데이터 타입의 경우는 이미 데이터가 존재하면 수정이 불가능 하다고 보면 됨.
     ...
     == 설계할 때 고려를 충분히 하자 ,,!
3. ****** 테이블이 이미 생성된 시점에서 제약조건을 추가 Through changing table *****
...
hp컬럼의 문자열 타입을 NUMBER 로 변경
ALTER TABLE emp_test MODIFY ( hp NUMBER(10) );

*** 데이터 타입을 바꾸는 것은 데이터가 없을 때는 상관이 없지만
    데이터가 있을 경우는 사이즈를 늘리는 것을 제외하고는 불가능 하다.
    
    
컬럼명 변경(데이터 타입 변경과 이름 변경은 자유롭다)
...
2. emp_test 테이블이 empno컬럼에 PRIMARY KEY 제약조건 추가
