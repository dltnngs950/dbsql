...
Exception (예외) : pl/sql 블럭이 실행되는동안 발생하는 에러이다.
                  예외가 발생했을 때 PL/SQL의 EXCEPTION 절을 기술하여
                  다른 로직을 실행하여 예외 문제를 해결할 수 있다.
예외 구분
1. 사전 정의 예외(Oracle predefined Exception)
    오라클에서 미리 정의한 예외로 ORA - XXXXX의 형태로 정의가 됨
    에러코드만 있고 에러이름이 없는 경우가 대다수 이다.
...
EXCEPTION
    ...
    WHEN 예외명3 [OR 예외명4] THEN
        실행 할 문장;
    WHEN OTHERS THEN
        실행 할 문장 (여기서 SQLCODE, SQLERRM 속성을 통해 예외코드와 메세지를 확인할 수 있다)
END;
/

하나의 행이 조회되어야 하는 상황에서 여러개의 행이 반환 된 경우 (예외처리가 없을 때)

SET SERVEROUTPUT ON; --출력 기능 활성화하는 문장.
...
사용자 예외 정의, 사용
1. PL/SQL 블럭 안에서 또 다른 PL/SQL 블럭을 정의 하는것이 가능하다.
...
분포도 : 흩어진 정도 -- 분포도에 따라 위 3가지 중 뭐가 더 나은지 알 수 있다.
데이터 분포도 : 특정 데이터가 발생하는 빈도
ex ) emp테이블의 empno 값이 14개 존재함 - 데이터 분포도가 좋다.
     emp테이블의 deptno 값이 3개 존재 - 데이터 분포도가 나쁘다.
     
함수의 경우 오라클에서 기본적으로 캐쉬 기능을 사용한다.
기본 캐쉬 사이즈가 20개이다.
getDeptName(10) => ACCOUNTING 
오라클은 10번 인자로 getDeptName을 실행 했을때 ACCOUNTING이라는 결과값을 기억(캐싱)
이후에 동일한 인자로 함수를 실행하면 함수를 실행하지 않고 캐싱된 값을 리턴한다.

이미 저장된 값을 가져오기때문에 중간에 값이 바뀌면 바뀐 값을 못가져올수도 있다 . 잘 판단 해야함 . (캐싱의 단점)
...
users 테이블의 pass 컬럼이 변경되었을 때 실행할 트리거
...
트리거
    ...
    : users 테이블에 비밀번호를 바꾸는 로직(java코드)은 작성을 해야한다
